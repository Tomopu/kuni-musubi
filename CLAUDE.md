...
- プログラムを書くときは、主要な処理手順を `1. ...`、`2. ...` の形式でコメントアウトして表記する。
- コメントは、各言語の通常のコメント記法を使う。例: TypeScript は `// 1. 入力を検証する`、Python は `# 1. 入力を検証する`。
- 既存コードを修正する場合は、変更対象の処理にだけ番号付きコメントを追加し、無関係な範囲へコメントを広げない。
